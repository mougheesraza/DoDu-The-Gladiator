...
export default async function handler(req: any, res: any) {
  try {
    ...
    const items = await facebookPageProvider.fetchLatestContent(Number.isFinite(limit) ? limit : 6);
    ...
    return res.status(500).json({ success: false, status: facebookPageProvider.getStatusDetails(), error: error?.message || 'Facebook API test failed' });
  }
}
